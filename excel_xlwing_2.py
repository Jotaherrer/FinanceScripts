...
def get_access(instrument=True):
    url1 = "https://api.invertironline.com/token"

    instrument = ['CEDEARs', 'Merval']
    data = {
            "username": usuario,
            "password": password,
            "grant_type": "password"        
    ...
    if response.status_code == 200:
        ...
        url2 = f'https://api.invertironline.com/api/v2/Cotizaciones/Acciones/{instrument[0]}/argentina'
        datos = requests.get(url2, headers={
                                        'Authorization': 'Bearer '+access_key
        })
        datos_texto = json.loads(datos.text)
        datos_use = datos_texto['titulos']
        datos_final = clean_assets(datos_use)
    return datos_final

def prices(tickers):
    """
    Function that gets daily prices of ADRs in NYSE. Pass a list with stock's tickers.
    """
    try:
...
while (hour >= 11) & (hour <= 20):
    ...
    local_stocks = ['BBAR','BMA', 'GGAL', 'PAMP','SUPV', 'YPFD']
    locales = get_access()

    stocks = ['AAPL','AMD','AMZN', 'BA','BABA', 'BBD', 'C','DESP','GOLD','HMY','INTC','JPM','KO','MELI','MSFT','NVDA','PBR', 'PFE','QCOM','TSLA','WFC']
    ...
    factores = [('AAPL',10),('AMD',0.5),('AMZN',72),('BA',3),('BABA',9),('BBD',1),('C', 3),('DESP',1),('GOLD',1),('HMY',1),('INTC', 5),('JPM',5),('KO',5),('MELI',2),('MSFT',5),('NVDA',12),('PBR',1),('PFE',2),('QCOM',11),('TSLA',15),('WFC', 5)]
